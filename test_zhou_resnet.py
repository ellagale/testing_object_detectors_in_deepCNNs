...
r.model_file = os.path.join(dir_root, 'zoo/resnet-152-torch-places365.caffemodel')
...
r.deploy_file = 'resnet-152-torch-places365.prototxt'
r.img_size = 224
...
with open("h5_list.txt","w") as fh:
    map(fh.write, glob.glob("*.h5"))


acts, h5_list = combine_h5_files_in_activation_table(h5_file_location=output_directory, #os.path.join(file_root, 'dataset/broden1_224/images/vgg_h5'),
