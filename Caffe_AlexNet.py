...
blob_list= ['conv5'] # ['prob','fc8', 'fc7', 'fc6']# ['prob'] #['fc8','fc6']
labels=[]
short_labels=[]
net = None
dir_list = ''
...
def convert_all_alexnet_to_h5_max(batches, blob_name, h5_out_filename, net, labels):
    ''' Parse the alexnet results and output the maximum activation for each correct match
    '''
    if verbose:
        print('Writing out {}'.format(h5_out_filename))
...
def convert_alexnet_to_h5_all(batches, blob_name, h5_out_filename, net, labels, forceLabels):
    ''' adds all images!'''
    if verbose:
        print('Writing out {}'.format(h5_out_filename))
...
def convert_alexnet_to_h5_max_all(batches, blob_name, h5_out_filename, net, labels):
    ''' adds all images!'''
    if verbose:
        print('Writing out {}'.format(h5_out_filename))
    # don't used accuracies here as we may not have them added!
