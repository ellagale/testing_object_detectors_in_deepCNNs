...
r.deploy_file = '/home/eg16993/neuralNetworks/experiments/NetDissect/NetDissect-release1/zoo/googlenet_places365.prototxt'
...
r.labels_file = os.path.join('/storage/data/ilsvrc12/',
                             'synset_words.txt')  # file with the class labels in it  should be ''class code' 'name'\n'
r.dir_list = os.path.join(
    '/storage/data/imagenet_2012_class_list.txt')  # the list of directoires which contain our images
...
h5_files_directory = os.path.join('/storage/data/Unmerged', 'GooglenetPlaces365Icep4e')
r.do_limited = True
first_run = False  # True #False # NTS run this once to make .h5 files, then put them in the h5_directory, amke h5_list.txt (to h5 direcotry) and rerun
if first_run:
    do_h5_files = True
else:
    do_h5_files = False
...
do_bus = True
do_car = False
figure_out_name = 'GoogleLeNetPlaces365LIncep4e' + str(neuron)
